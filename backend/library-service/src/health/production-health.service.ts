...
export class ProductionHealthService {
    ...
    try {
      ...
      if (performanceCheck.status === 'fulfilled') {
        ...
        results.performance = {
          performance: {
            status: 'down',
            error:
              performanceCheck.reason?.message || 'Performance check failed',
          },
        };
        statuses.push('down');
      }
      ...
      if (securityCheck.status === 'fulfilled') {
        ...
        results.security = {
          security: {
            status: 'down',
            error: securityCheck.reason?.message || 'Security check failed',
          },
        };
        statuses.push('down');
      }
      ...
      if (resourcesCheck.status === 'fulfilled') {
        ...
        results.resources = {
          resources: {
            status: 'down',
            error: resourcesCheck.reason?.message || 'Resources check failed',
          },
        };
        statuses.push('down');
      }
      ...
      if (externalCheck.status === 'fulfilled') {
        ...
        results.external = {
          external: {
            status: 'down',
            error:
              externalCheck.reason?.message || 'External services check failed',
          },
        };
        statuses.push('down');
      }
  ...
  private async checkSystemHealth(): Promise<HealthIndicatorResult> {
    try {
      ...
      if (!this.isNodeVersionCompatible(nodeVersion, requiredNodeVersion)) {
        throw new Error(
          `Node.js version ${nodeVersion} is not compatible with required ${requiredNodeVersion}`,
        );
      }
      ...
      if (diskSpace.available < 1024 * 1024 * 1024) {
        // Less than 1GB
        throw new Error(
          `Low disk space: ${Math.round(diskSpace.available / 1024 / 1024)}MB available`,
        );
      }
  ...
  private async checkPerformanceHealth(): Promise<HealthIndicatorResult> {
    try {
      ...
      if (current.cpu.percent > 90) {
        throw new Error(
          `Critical CPU usage: ${current.cpu.percent.toFixed(1)}%`,
        );
      }

      if (current.eventLoop.lag > 200) {
        throw new Error(
          `Critical event loop lag: ${current.eventLoop.lag.toFixed(1)}ms`,
        );
      }

      if (alerts.critical > 0) {
        throw new Error(
          `${alerts.critical} critical performance alerts active`,
        );
      }
  ...
  private async checkSecurityHealth(): Promise<HealthIndicatorResult> {
    try {
      ...
      if (secretsStatus.missing.length > 0) {
        throw new Error(
          `Missing required secrets: ${secretsStatus.missing.join(', ')}`,
        );
      }
      ...
      if (isProduction && hasWeakSecrets) {
        ...
        throw new Error(
          `Weak secrets detected: ${secretsStatus.weak.join(', ')}`,
        );
      }
      ...
      if (!secretsValidation.valid) {
        throw new Error(
          `Secret validation failed: ${secretsValidation.errors.join(', ')}`,
        );
      }
      ...
      return {
        security: {
          ...
          warnings:
            hasWeakSecrets || hasSecurityIssues
              ? [
                  ...(hasWeakSecrets
                    ? [`Weak secrets: ${secretsStatus.weak.join(', ')}`]
                    : []),
                  ...securityIssues,
                ].join('; ')
              : undefined,
  ...
  private async checkResourcesHealth(): Promise<HealthIndicatorResult> {
    try {
      ...
      const hasResourceWarnings =
        activeHandles > 1000 || activeRequests > 100 || gcStats.duration > 100;
  ...
  private async checkExternalServicesHealth(): Promise<HealthIndicatorResult> {
    try {
      ...
      if (failedChecks.length > 0) {
        throw new Error(
          `External service checks failed: ${failedChecks.join(', ')}`,
        );
      }
