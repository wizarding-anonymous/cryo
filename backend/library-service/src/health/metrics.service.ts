...
export class MetricsService {
  ...
  recordExternalServiceResponseTime(service: string, responseTime: number): void {
    this.externalServiceResponseTimeHistogram.observe({ service }, responseTime / 1000); // Convert to seconds
  }
  ...
  recordCacheOperation(operation: string, status: 'hit' | 'miss' | 'error'): void {
    this.cacheOperationsCounter.inc({ operation, status });
  }

  setCircuitBreakerState(service: string, state: 'CLOSED' | 'OPEN' | 'HALF_OPEN'): void {
