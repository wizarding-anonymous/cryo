...
export default registerAs('performance', (): PerformanceConfig => ({
  ...
  cache: {
    ...
    cacheWarmingInterval: parseInt(process.env.CACHE_WARMING_INTERVAL || '300000', 10), // 5 minutes
  ...
  application: {
    ...
    maxBulkInsertSize: parseInt(process.env.MAX_BULK_INSERT_SIZE || '1000', 10),
    bulkInsertBatchSize: parseInt(process.env.BULK_INSERT_BATCH_SIZE || '100', 10),
    ...
    enablePerformanceMonitoring: process.env.PERFORMANCE_MONITORING_ENABLED !== 'false',
    performanceMetricsInterval: parseInt(process.env.PERFORMANCE_METRICS_INTERVAL || '30000', 10),
    maxMetricsHistory: parseInt(process.env.MAX_METRICS_HISTORY || '10000', 10),
  },
  ...
  optimization: {
    ...
    enablePreparedStatements: process.env.PREPARED_STATEMENTS_ENABLED !== 'false',
    enableBatchQueries: process.env.BATCH_QUERIES_ENABLED === 'true',
    enableAutoIndexing: process.env.AUTO_INDEXING_ENABLED === 'true',
    indexMaintenanceInterval: parseInt(process.env.INDEX_MAINTENANCE_INTERVAL || '3600000', 10), // 1 hour
    enableMaterializedViews: process.env.MATERIALIZED_VIEWS_ENABLED !== 'false',
    materializedViewRefreshInterval: parseInt(process.env.MATERIALIZED_VIEW_REFRESH_INTERVAL || '1800000', 10), // 30 minutes
    enableResponseCompression: process.env.RESPONSE_COMPRESSION_ENABLED !== 'false',
    compressionThreshold: parseInt(process.env.COMPRESSION_THRESHOLD || '1024', 10), // 1KB
