...
function loadSecret(secretName: string, envVar?: string): string {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      throw new Error(
        `Secret '${secretName}' not found in file or environment variable`,
      );
    }

    console.warn(
      `Warning: Secret '${secretName}' not found, using default value`,
    );
  ...
  return {
    ...
    services: {
      gamesCatalog: {
        ...
        retryDelay: parseInt(
          process.env.GAMES_CATALOG_RETRY_DELAY || '1000',
          10,
        ),
      },
      payment: {
        ...
        retryDelay: parseInt(
          process.env.PAYMENT_SERVICE_RETRY_DELAY || '1000',
          10,
        ),
      },
      user: {
        ...
        retryDelay: parseInt(
          process.env.USER_SERVICE_RETRY_DELAY || '1000',
          10,
        ),
      },
    },
    ...
    apm: {
      ...
      spanFramesMinDuration: parseInt(
        process.env.ELASTIC_APM_SPAN_FRAMES_MIN_DURATION || '5',
        10,
      ),
      stackTraceLimit: parseInt(
        process.env.ELASTIC_APM_STACK_TRACE_LIMIT || '50',
        10,
      ),
    },
