...
describe('OptionalAuthGuard', () => {
  ...
  it('validates when token present', async () => {
    ...
    await expect(
      guard.canActivate(makeCtx({ authorization: 'Bearer t' })),
    ).resolves.toBe(true);
  ...
  it('throws 401 on invalid token', async () => {
    const guard = new OptionalAuthGuard({
      validateBearerToken: jest.fn().mockRejectedValue(new Error()),
    } as any);
    await expect(
      guard.canActivate(makeCtx({ authorization: 'Bearer bad' })),
