  ...
  it('returns cached payload when present', (done) => {
    const client = {
      get: jest.fn().mockResolvedValue(
        JSON.stringify({ statusCode: 200, headers: { 'content-type': 'application/json' }, body: { ok: true } }),
      ),
    } as any;
    const interceptor = new CacheInterceptor({ get: () => true } as any, { getClient: () => client } as any);
    const { ctx, res } = make();
    interceptor.intercept(ctx, { handle: () => of('not-used') } as any).subscribe({
      complete: () => {
