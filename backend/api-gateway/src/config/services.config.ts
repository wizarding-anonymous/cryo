...
export default registerAs('services', (): ServicesConfig => {
  ...
  return {
    'user-service': build(
      'user-service',
      process.env.SERVICE_USER_BASE_URL ?? 'http://localhost:3000',
    ),
    ...
    'payment-service': build(
      'payment-service',
      process.env.SERVICE_PAYMENT_BASE_URL ?? 'http://localhost:3003',
    ),
    'library-service': build(
      'library-service',
      process.env.SERVICE_LIBRARY_BASE_URL ?? 'http://localhost:3004',
    ),
    ...
    'review-service': build(
      'review-service',
      process.env.SERVICE_REVIEW_BASE_URL ?? 'http://localhost:3006',
    ),
    ...
    'security-service': build(
      'security-service',
      process.env.SERVICE_SECURITY_BASE_URL ?? 'http://localhost:3008',
    ),
    'social-service': build(
      'social-service',
      process.env.SERVICE_SOCIAL_BASE_URL ?? 'http://localhost:3009',
    ),
    'download-service': build(
      'download-service',
      process.env.SERVICE_DOWNLOAD_BASE_URL ?? 'http://localhost:3010',
    ),
    'api-gateway': build(
      'api-gateway',
