...
describe('Health Module Integration', () => {
  ...
  describe('GET /metrics', () => {
    ...
    it('should return metrics in Prometheus format', async () => {
      ...
      const hasHelpLines = lines.some((line: string) =>
        line.startsWith('# HELP'),
      );
      const hasTypeLines = lines.some((line: string) =>
        line.startsWith('# TYPE'),
      );
      const hasMetricLines = lines.some((line: string) =>
        line.match(/^gateway_\w+/),
      );
  ...
  describe('Error handling', () => {
    it('should handle 404 for non-existent health endpoints', async () => {
      await request(app.getHttpServer()).get('/health/nonexistent').expect(404);
    });

    it('should handle invalid HTTP methods', async () => {
      await request(app.getHttpServer()).post('/health').expect(404);

      await request(app.getHttpServer()).put('/health/services').expect(404);

      await request(app.getHttpServer()).delete('/metrics').expect(404);
    });
  });
