...
export class WebhookController {
  ...
  async handleWebhook(
    ...
    this.logger.log(
      `Received webhook from ${provider} with payload: ${JSON.stringify(webhookDto)}`,
    );

    const { externalId, status } = this.paymentProviderService.handleWebhook(
      provider,
      webhookDto,
    );
    ...
    if (!externalId) {
      throw new BadRequestException(
        'Could not extract externalId from webhook payload.',
      );
    }
    ...
    if (!payment) {
      throw new NotFoundException(
        `Payment with external ID ${externalId} not found.`,
      );
    }
