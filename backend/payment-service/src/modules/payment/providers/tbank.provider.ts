...
@Injectable()
export class TBankPaymentProvider implements PaymentProviderInterface {
  constructor(private readonly configService: ConfigService) {}

  async processPayment(payment: Payment): Promise<ProcessPaymentResponse> {
    const externalId = `tbank_${uuidv4()}`;
    const baseUrl = this.configService.get<string>('APP_URL');
    const paymentUrl = `${baseUrl}/payments/${payment.id}/mock-form`;

    console.log(`TBANK_PROVIDER: Processing payment ${payment.id}, assigned externalId ${externalId}`);

    return { paymentUrl, externalId };
  }

  async getPaymentStatus(externalId: string): Promise<{ status: string; providerResponse: any }> {
    console.log(`TBANK_PROVIDER: Getting status for externalId ${externalId}`);
    return {
  ...
  handleWebhook(data: any): { externalId: string; status: string } {
    console.log(`TBANK_PROVIDER: Handling webhook`);
    return {
