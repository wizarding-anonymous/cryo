...
# Функция для создания тестового пользователя через User Service
function Create-TestUserInUserService {
    Write-Host "👤 Создание тестового пользователя в User Service..." -ForegroundColor Yellow
    
    $userData = @{
        email = $TEST_EMAIL
        name = "Migration Test User"
        passwordHash = '$2b$10$example.hash.for.migration.test'  # Пример хеша
    }
    
    $result = Invoke-ApiRequest -Url "$USER_SERVICE_URL/users" -Method "POST" -Body $userData
    
    if ($result.Success) {
        Write-Host "  ✅ Пользователь создан в User Service: $($result.Data.id)" -ForegroundColor Green
        return $result.Data
    } else {
        Write-Host "  ❌ Ошибка создания пользователя: $($result.Error)" -ForegroundColor Red
        return $null
    }
}
...
function Test-AuthUserServiceIntegration {
    ...
    if ($registerResult.Success) {
        ...
        # Тест 4: Логин с существующими данными
        Write-Host "  🔑 Тест 4: Логин с существующими данными..." -ForegroundColor Cyan
        ...
        $loginResult = Invoke-ApiRequest -Url "$AUTH_SERVICE_URL/api/auth/login" -Method "POST" -Body $loginData
        
        if ($loginResult.Success) {
            Write-Host "    ✅ Логин успешен: $($loginResult.Data.user.email)" -ForegroundColor Green
        } else {
            Write-Host "    ❌ Логин неуспешен: $($loginResult.Error)" -ForegroundColor Red
            return $false
        }
...
# Функция для проверки Redis и PostgreSQL интеграции
function Test-DatabaseIntegration {
    Write-Host "🗄️ Проверка интеграции с базами данных..." -ForegroundColor Yellow
    
    # Создаем пользователя и проверяем сохранение в БД
    $registerData = @{
        email = "db-test-$(Get-Date -Format 'yyyyMMdd-HHmmss')@example.com"
        password = $TEST_PASSWORD
    ...
    if ($registerResult.Success) {
        ...
        if ($loginResult.Success) {
            Write-Host "  ✅ Сессия создана в PostgreSQL" -ForegroundColor Green
            
            # Logout для проверки blacklist в Redis
            $headers = @{ "Authorization" = "Bearer $accessToken" }
            $logoutResult = Invoke-ApiRequest -Url "$AUTH_SERVICE_URL/api/auth/logout" -Method "POST" -Headers $headers
            
            if ($logoutResult.Success) {
                Write-Host "  ✅ Токен добавлен в blacklist (Redis)" -ForegroundColor Green
                
                # Проверяем что токен теперь невалиден
                $validateResult = Invoke-ApiRequest -Url "$AUTH_SERVICE_URL/api/auth/validate" -Method "POST" -Body @{ token = $accessToken }
                
                if (-not $validateResult.Success) {
                    Write-Host "  ✅ Blacklist работает - токен невалиден после logout" -ForegroundColor Green
                    return $true
                } else {
                    Write-Host "  ❌ Blacklist не работает - токен все еще валиден" -ForegroundColor Red
                    return $false
                }
            } else {
                Write-Host "  ❌ Logout не работает: $($logoutResult.Error)" -ForegroundColor Red
                return $false
            }
        } else {
            Write-Host "  ❌ Login не работает: $($loginResult.Error)" -ForegroundColor Red
            return $false
        }
    } else {
        Write-Host "  ❌ Регистрация не работает: $($registerResult.Error)" -ForegroundColor Red
        return $false
    }
}
...
function Main {
    ...
    # Тест 5: Интеграция с базами данных
    Write-Host "=== ЭТАП 5: ИНТЕГРАЦИЯ С БАЗАМИ ДАННЫХ ===" -ForegroundColor Magenta
    $dbPassed = Test-DatabaseIntegration
    if (-not $dbPassed) { $allTestsPassed = $false }
    ...
    if ($allTestsPassed) {
        ...
        Write-Host "✅ Базы данных интегрированы корректно" -ForegroundColor Green
        return $true
    } else {
