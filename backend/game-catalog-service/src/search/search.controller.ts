import { Controller, Get, Query, UseInterceptors, HttpException, HttpStatus, Logger } from '@nestjs/common';
...
export class SearchController {
  ...
  @ApiOperation({
    ...
    description: 'Search games using PostgreSQL full-text search with Russian language support. Supports pagination, price filtering, and different search types.'
  })
  @ApiQuery({
    name: 'q',
    required: false,
      ...
      properties: {
        error: {
          ...
          properties: {
            ...
            message: { type: 'string', example: 'Search query must be at least 1 character long' },
            details: { type: 'object' }
          }
        }
      }
  ...
  async searchGames(
    ...
    try {
      ...
      if (searchGamesDto.minPrice !== undefined && searchGamesDto.maxPrice !== undefined) {
      ...
      if (searchGamesDto.minPrice !== undefined && searchGamesDto.minPrice < 0) {
        throw new HttpException(
      ...
      if (searchGamesDto.maxPrice !== undefined && searchGamesDto.maxPrice < 0) {
        throw new HttpException(
