...
export class DatabaseModule implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (!redisConfigValid) {
      this.logger.warn('Redis configuration validation failed, using memory cache fallback');
    }
    ...
    try {
      ...
      if (!schemaValid) {
        this.logger.warn('Database schema validation failed - migrations may be needed');
      }
    } catch (error) {
      this.logger.warn('Could not validate database schema on startup', error.message);
    }

    this.logger.log('Database module initialized successfully');
    this.logger.log(`Database: ${this.databaseConfigService.getConnectionInfo()}`);
