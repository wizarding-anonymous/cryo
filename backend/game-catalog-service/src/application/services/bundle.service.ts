...
export class BundleService {
  ...
  async getBundleForUser(id: string, userId: string) {
    const bundle = await this.findOne(id);
    const gameIds = bundle.games.map(game => game.id);

    const ownedGameIds = await this.libraryService.filterOwnedGames(userId, gameIds);

    const gamesNotInLibrary = bundle.games.filter(game => !ownedGameIds.includes(game.id));

    // Naive dynamic price calculation. A real implementation would be more complex.
    const dynamicPrice = gamesNotInLibrary.reduce((total, game) => total + game.price, 0);

    const totalOriginalPrice = bundle.games.reduce((total, game) => total + game.price, 0);
    const savings = totalOriginalPrice - bundle.price;

    return {
      ...bundle,
      games: gamesNotInLibrary, // Only show games the user doesn't own
      price: dynamicPrice, // The price for the remaining items
      originalBundlePrice: bundle.price,
      savings: savings,
    };
  }
}
