...
import { setupTestDatabase, seedTestData, cleanupTestDatabase } from './setup-e2e';
...
describe('Database Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        const additionalGames = allGames.filter(game => !testGameIds.includes(game.id));
    ...
    describe('Database Connection and Schema', () => {
        ...
        it('should support database transactions', async () => {
            ...
            const games = await gameRepository.find({ where: { title: 'Transaction Test Game' } });
    ...
    describe('Game Repository Operations', () => {
        ...
        it('should update game fields correctly', async () => {
            ...
            expect(updatedGame.updatedAt.getTime()).toBeGreaterThan(originalUpdatedAt.getTime());
        });
        ...
        it('should handle concurrent updates correctly', async () => {
            ...
            const update1 = gameRepository.save({ ...game, title: 'Concurrent Update 1' });
            const update2 = gameRepository.save({ ...game, title: 'Concurrent Update 2' });

            const [result1, result2] = await Promise.all([update1, update2]);

            // One of the updates should succeed
            expect(result1.title === 'Concurrent Update 1' || result2.title === 'Concurrent Update 2').toBe(true);
        });
    });
    ...
    describe('GameService Database Integration', () => {
        ...
        it('should handle database errors gracefully', async () => {
            ...
            await expect(gameService.createGame(invalidGameData as any)).rejects.toThrow();
        });
    });
    ...
    describe('SearchService Database Integration', () => {
        it('should perform full-text search on games', async () => {
            ...
            const hasSearchTerm = searchResult.games.some(game =>
        ...
        it('should search by specific fields', async () => {
            ...
            const foundGame = titleSearch.games.find(game => game.title === 'Test Game 1');
    ...
    describe('Data Integrity and Constraints', () => {
        ...
        it('should validate data types and constraints', async () => {
            ...
            expect(result.price).toBe(-10.00); // Adjust based on validation rules
        });
    });
});