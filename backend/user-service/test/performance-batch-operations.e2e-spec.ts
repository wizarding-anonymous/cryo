...
describe('Performance Batch Operations Tests (e2e)', () => {
  ...
  describe('Large Batch Creation Performance (within limits)', () => {
    it('should handle batch creation of 1,000 users efficiently', async () => {
      const batchSize = 1000; // Within 5,000 limit
    ...
    it('should handle batch creation of 2,500 users with optimal chunking', async () => {
      const batchSize = 2500; // Within 5,000 limit
      const chunkSize = 250; // Larger chunks for better performance
  ...
  describe('Large Batch Lookup Performance', () => {
    beforeEach(async () => {
      ...
      const testUsers = Array.from({ length: 1500 }, (_, i) => ({
    ...
    it('should handle batch lookup of 1,000 users efficiently', async () => {
    ...
    it('should handle paginated batch lookup efficiently', async () => {
      ...
      for (let page = 1; page <= totalPages; page++) {
        ...
        const response = await request(app.getHttpServer())
          .get(`/api/batch/users?page=${page}&limit=${pageSize}&sortBy=createdAt&sortOrder=asc`)
          .set('x-internal-service', 'user-service-internal')
          .expect(200);
        ...
        console.log(`  📄 Page ${page} completed in ${pageDuration}ms (${response.body.data.length} users)`);

        expect(response.body.data.length).toBeGreaterThan(0);
        expect(response.body.data.length).toBeLessThanOrEqual(pageSize);
        allResults.push(...response.body.data);
  ...
  describe('Large Batch Update Performance', () => {
    ...
    beforeEach(async () => {
      // Create test data for update tests (within 5,000 limit)
      const testUsers = Array.from({ length: 4000 }, (_, i) => ({
    ...
    it('should handle batch update of 3,000 users efficiently', async () => {
      const updateUserIds = testUserIds.slice(0, 3000);
    ...
    it('should handle batch soft delete of large number of users', async () => {
      const deleteUserIds = testUserIds.slice(0, 2000);
