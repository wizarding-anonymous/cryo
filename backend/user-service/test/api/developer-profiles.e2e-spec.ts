...
describe('Developer Profiles API (e2e)', () => {
  ...
  it('GET /developers/:userId/basic-profile should return profile', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/developers/${userId}/basic-profile`).expect(200);
  ...
  it('GET /developers/:userId/verification-status should return status', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/developers/${userId}/verification-status`).expect(200);
