...
### Требование 21: Интеграция с Preorder Service

**Пользовательская история:** Как пользователь каталога, я хочу видеть информацию о предзаказах игр, чтобы понимать доступные варианты покупки.

#### Критерии приемки

1. КОГДА игра имеет доступные предзаказы ТОГДА система ДОЛЖНА получить информацию из Preorder Service и отобразить статус
2. КОГДА пользователь просматривает игру ТОГДА система ДОЛЖНА показать доступные уровни предзаказа из Preorder Service
3. КОГДА пользователь хочет оформить предзаказ ТОГДА система ДОЛЖНА перенаправить запрос в Preorder Service
4. КОГДА Preorder Service уведомляет о выполнении предзаказа ТОГДА система ДОЛЖНА обновить статус игры
5. ЕСЛИ Preorder Service недоступен ТОГДА система ДОЛЖНА показать базовую информацию об игре без предзаказов
...
### Требование 25: Интеграция с Game Keys Service

**Пользовательская история:** Как пользователь каталога, я хочу видеть информацию о возможности активации ключей для игр, чтобы понимать доступные способы получения игры.

#### Критерии приемки

1. КОГДА пользователь просматривает игру ТОГДА система ДОЛЖНА получить из Game Keys Service информацию о поддержке активации ключей
2. КОГДА игра поддерживает активацию ключей ТОГДА система ДОЛЖНА отобразить соответствующую информацию в каталоге
3. КОГДА пользователь хочет активировать ключ ТОГДА система ДОЛЖНА перенаправить запрос в Game Keys Service
4. КОГДА Game Keys Service уведомляет об активации ключа ТОГДА система ДОЛЖНА обновить статистику игры
5. ЕСЛИ Game Keys Service недоступен ТОГДА система ДОЛЖНА скрыть информацию об активации ключей

### Требование 26: Интеграция с Greenlight Service

**Пользовательская история:** Как разработчик, я хочу подавать свою игру через систему Greenlight и отображать статус в каталоге, чтобы показать одобрение сообщества.

#### Критерии приемки

1. КОГДА разработчик хочет подать игру на Greenlight ТОГДА система ДОЛЖНА отправить запрос в Greenlight Service
2. КОГДА игра находится на голосовании ТОГДА система ДОЛЖНА получать статус из Greenlight Service и отображать его в каталоге
3. КОГДА игра одобрена через Greenlight ТОГДА система ДОЛЖНА получить уведомление и отметить игру статусом "Greenlight Graduate"
4. КОГДА пользователь просматривает Greenlight игру ТОГДА система ДОЛЖНА показать статус голосования из Greenlight Service
5. ЕСЛИ Greenlight Service недоступен ТОГДА система ДОЛЖНА скрыть информацию о статусе Greenlight
...
### Требование 30: Система версионирования и обновлений игр

**Пользовательская история:** Как владелец игры, я хочу видеть информацию об обновлениях и версиях игр в каталоге, чтобы быть в курсе изменений.

#### Критерии приемки

1. КОГДА разработчик загружает новую версию игры ТОГДА система ДОЛЖНА создать новую запись версии с changelog
2. КОГДА выходит обновление ТОГДА система ДОЛЖНА отображать changelog и дату обновления на странице игры
3. КОГДА пользователь просматривает игру ТОГДА система ДОЛЖНА показывать текущую версию и историю обновлений
4. КОГДА обновление критическое ТОГДА система ДОЛЖНА помечать его соответствующим статусом
5. ЕСЛИ игра в раннем доступе ТОГДА система ДОЛЖНА отображать roadmap разработки и планы обновлений

### Требование 31: Система статусов жизненного цикла игр

**Пользовательская история:** Как пользователь каталога, я хочу видеть актуальный статус разработки игры, чтобы понимать стадию ее готовности.

#### Критерии приемки

1. КОГДА игра добавляется в каталог ТОГДА система ДОЛЖНА установить соответствующий статус: draft, alpha, beta, early_access, coming_soon, released
2. КОГДА игра находится в раннем доступе ТОГДА система ДОЛЖНА отображать предупреждение и roadmap разработки
3. КОГДА статус игры изменяется ТОГДА система ДОЛЖНА обновить отображение и уведомить заинтересованных пользователей
4. КОГДА пользователь фильтрует игры ТОГДА система ДОЛЖНА поддерживать фильтрацию по статусу разработки
5. ЕСЛИ игра снята с продажи ТОГДА система ДОЛЖНА изменить статус на discontinued и скрыть возможность покупки