...
import { DeveloperProfileUpdatedEvent } from '../events/schemas/developer-profile-updated.event.ts';
import { DeveloperVerificationChangedEvent } from '../events/schemas/developer-verification-changed.event.ts';
...
export class BasicDeveloperService {
  ...
  async updateBasicDeveloperProfile(userId: string, updates: Partial<CreateDeveloperProfileDto>): Promise<DeveloperProfile> {
  ...
  async updateVerificationStatus(userId: string, newStatus: VerificationStatus): Promise<DeveloperProfile> {
    const profile = await this.developerProfileRepository.findOneBy({ userId });
    if (!profile) {
      throw new NotFoundException(`Developer profile for user ID ${userId} not found`);
    }
    ...
    profile.isVerified = newStatus === VerificationStatus.Approved;
    ...
    await this.eventPublisher.publish(
        ...
        new DeveloperVerificationChangedEvent({
            userId: updatedProfile.userId,
            developerId: updatedProfile.id,
            newStatus: updatedProfile.verificationStatus as VerificationStatus,
            timestamp: new Date().toISOString(),
        }),
